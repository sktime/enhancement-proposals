...
from .tsf_awkward import TimeSeriesForest_ak_3d
from .tsf_awkward import TimeSeriesForest_ak_record
...
def test_tsf_ak_record(benchmark):
    ...
    estimator = TimeSeriesForest_ak_record(random_state=1)
...
def test_tsf_ak_3d(benchmark):
    ...
    estimator = TimeSeriesForest_ak_3d(random_state=1)
    actual = benchmark(_fit_predict, estimator, X_train_ak, y_train, X_test_ak)
    np.testing.assert_array_equal(actual, expected)
